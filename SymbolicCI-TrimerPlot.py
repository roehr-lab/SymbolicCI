...
def ETT(s,F,eri):
    ...
    #print(groundEn)
    return s
def plot_fct(del_X=0.0,del_Y=0.0, color="ocean_r", nameIMG=""):
    vbs = del_X
    mvs = del_Y
    col = color
    v = del_X
...
#h = fron.T.mm(hcore).mm(fron)
    ...
    mt = torch.cat([s[3:9,:3],s[3:9,9:]], dim=1)
    print(mt)
    torch.save(mt,"OnlyFock_%f_%f.pt"%(v,mvi))
    plt.title("Fock Matrix Only")
    plt.imshow(mt.abs(),cmap=plt.get_cmap("cividis_r"))
    ...
    #r'$\left|S_{0}S_{0}S_{0} \right\rangle$',
                           r'$|S_{1}S_{0}S_{0} \rangle$', 
                          r'$|S_{0}S_{1}S_{0} \rangle$', 
                          r'$|S_{0}S_{0}S_{1} \rangle$', 
                          #r'$\left|C_{0}A_{0}S_{0} \right\rangle$', 
                         #r'$\left|S_{0}C_{0}A_{0} \right\rangle$',
                         #r'$\left|A_{0}S_{0}C_{0} \right\rangle$',
                         #r'$\left|A_{0}C_{0}S_{0} \right\rangle$',
                         #r'$\left|S_{0}A_{0}C_{0} \right\rangle$',
                         #r'$\left|C_{0}S_{0}A_{0} \right\rangle$',
                         r'$|T_{0}T_{0}S_{0} \rangle$',
                         r'$|S_{0}T_{0}T_{0} \rangle$',
                         r'$|T_{0}S_{0}T_{0} \rangle$'),rotation = 90)


    plt.yticks(np.arange(6),
           (
               #r'$\left|S_{1}S_{0}S_{0} \right\rangle$', 
               #          r'$\left|S_{0}S_{1}S_{0} \right\rangle$', 
               #           r'$\left|S_{0}S_{0}S_{1} \right\rangle$', 
                          r'$\left|C_{0}A_{0}S_{0} \right\rangle$', 
                         r'$\left|S_{0}C_{0}A_{0} \right\rangle$',
                         r'$\left|A_{0}S_{0}C_{0} \right\rangle$',
                         r'$\left|A_{0}C_{0}S_{0} \right\rangle$',
                         r'$\left|S_{0}A_{0}C_{0} \right\rangle$',
                         r'$\left|C_{0}S_{0}A_{0} \right\rangle$'),
                        #r'$\left|T_{0}T_{0}S_{0} \right\rangle$',
                        # r'$\left|S_{0}T_{0}T_{0} \right\rangle$',
                        # r'$\left|T_{0}S_{0}T_{0} \right\rangle$'),
               )
    x = torch.arange(0.0,6,1)
    y = torch.arange(0.0,6,1)
    dxx,dyy = torch.meshgrid(x,y)

    slx = dxx.reshape(-1)
    sly = dyy.reshape(-1)
    m = ["%2.3f"%(mt[int(mi%6),int(mi/6)]*27.2114) for mi in range(36)]
    for mi in range(len(m)):
        if(abs(mt[int(mi%6),int(mi/6)]) >0.000000000000001):
            if(abs(s[int(mi%6),int(mi/6)]) >0.1):
                plt.text(slx[mi]-0.4,sly[mi]+0.2,m[mi], color="white")
            else:
                plt.text(slx[mi]-0.4,sly[mi]+0.2,m[mi], color="White")
    ...
    plt.figure(dpi=100,figsize=(10,10))
    mt = torch.cat([s[3:9,:3],s[3:9,9:]], dim=1)
    torch.save(mt,"OnlyEri_%f_%f.pt"%(v,mvi))
    print(mt)
    plt.title("Electron Repulsion Integrals Only")
    plt.imshow(mt.abs(),cmap=plt.get_cmap("cividis_r"))
    plt.tick_params(bottom=False, top=True, left=True, right=False)
    plt.tick_params(labelbottom=False, labeltop=True, labelleft=True, labelright=False)
    plt.xticks(np.arange(6), (
...
#plt.show()
    ...
    plt.figure(dpi=100,figsize=(10,10))
    mt = torch.cat([s[3:9,:3],s[3:9,9:]], dim=1)
    torch.save(mt,"Both_%f_%f.pt"%(v,mvi))
    print(mt)
    plt.title("Fock + ERI ")
    plt.imshow(mt.abs(),cmap=plt.get_cmap("cividis_r"))
    plt.tick_params(bottom=False, top=True, left=True, right=False)
    ...
    plt.show()
    plt.figure(dpi=100,figsize=(24,20))
    plt.imshow(torch.abs(s )*27.2114,cmap=plt.get_cmap("cividis_r"))#.reversed() )
    plt.colorbar()
    plt.title("Coupling Full")
    plt.tick_params(bottom=False, top=True, left=True, right=False)
    plt.tick_params(labelbottom=False, labeltop=True, labelleft=True, labelright=False)
    ...
    x = torch.arange(0.0,12,1)
    y = torch.arange(0.0,12,1)
    dxx,dyy = torch.meshgrid(x,y)

    slx = dxx.reshape(-1)
    sly = dyy.reshape(-1)
    m = ["%2.3f"%(s[int(mi%12),int(mi/12)]*27.2114) for mi in range(144)]
    for mi in range(len(m)):
        if(abs(s[int(mi%12),int(mi/12)]) >0.000000000000001):
            if(abs(s[int(mi%12),int(mi/12)]) >0.1):
                plt.text(slx[mi]-0.4,sly[mi]+0.2,m[mi], color="white")
            else:
                plt.text(slx[mi]-0.4,sly[mi]+0.2,m[mi], color="White")
#plt.yticks(np.arange(13),["%d :: %3.2feV :: %2.2feV"%(i, kb[i],kb[i]- kb.min() ) for i in np.arange(13)])
    ...
    print("Diabtic Energy")
    print(torch.diagonal(bd*27.2114))
    ...
    #return kb, ac.T
    plt.figure(dpi=100,figsize=(24,20))
    #plt.imshow(ac.T.abs(),cmap=plt.get_cmap("copper"))#.reversed() )
    plt.imshow(ac.T.abs(),cmap=plt.get_cmap(color))
    plt.colorbar()
    #plt.title("Admixture in Adiabatic wave function as x = %2.2f & y = %2.2f"%(v,mvi), fontsize=18)
    plt.title(r"Admixture in Adiabatic wave function at $\Delta x = %f \AA$"%(del_X))
    
    plt.tick_params(bottom=False, top=True, left=True, right=False)
    plt.tick_params(labelbottom=False, labeltop=True, labelleft=True, labelright=False)
    plt.xticks(np.arange(13), (r'$\left|S_{0}S_{0}S_{0} \right\rangle$',
                           r'$\left|S_{1}S_{0}S_{0} \right\rangle$', 
                          r'$\left|S_{0}S_{1}S_{0} \right\rangle$', 
                          r'$\left|S_{0}S_{0}S_{1} \right\rangle$', 
                          r'$\left|C_{0}A_{0}S_{0} \right\rangle$', 
                         ...
                         r'$\left|T_{0}S_{0}T_{0} \right\rangle$'))
    sdt = ac.T
    x = torch.arange(0.0,13,1)
    y = torch.arange(0.0,13,1)
    dxx,dyy = torch.meshgrid(x,y)

    slx = dxx.reshape(-1)
    sly = dyy.reshape(-1)

    m = ["%2.2e"%(sdt[int(mi%13),int(mi/13)]) for mi in range(169)]
    for mi in range(len(m)):
        if(abs(sdt[int(mi%13),int(mi/13)]) >0.000000000000001):
            if(abs(sdt[int(mi%13),int(mi/13)]) >0.1):
                plt.text(slx[mi]-0.1,sly[mi],m[mi], color="white")
            else:
                plt.text(slx[mi]-0.1,sly[mi],m[mi], color="White")
    plt.yticks(np.arange(13),["%d :: %3.2feV :: %2.2feV"%(i, kb[i],kb[i]- kb.min() ) for i in np.arange(13)])
    plt.show()
